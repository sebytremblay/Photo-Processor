...
public class Mosaicking implements ImageProcessingCommand {
  ...
  /**
   * Creates a command the mosaicks an image with the given amount of seeds.
   *
   * @param numSeeds the number of seeds in the resulting mosaick
   */
  ...
  /**
   * Creates a mosaicking command with a provided number of seeds and a
   * strategy on how to choose the seeds from the image.
   *
   * @param numSeeds the number of seeds
   * @param func     the choosing seeds strategy
   */
  public Mosaicking(int numSeeds, Function<ImageModel, List<PixelCoordinate>> func) {
    Util.requireNonNullArg(func);
  ...
  private ImageModel doMosaick(Map<PixelCoordinate,
          List<PixelCoordinate>> seedMapping, ImageModel m) {
